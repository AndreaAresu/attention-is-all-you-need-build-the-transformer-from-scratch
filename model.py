...
def build_position_index_column(max_len):
    ...
    return torch.reshape((torch.arange(float(max_len))), (max_len, 1))
...
def build_sinusoidal_positional_encoding(max_len, d_model):
    ...
    out = fill_odd_indices_with_cos(pe, position, div_term)
    return out
...
def add_positional_encoding_to_embeddings(embedded_batch, positional_encoding):
    return torch.add(embedded_batch, positional_encoding[:embedded_batch.shape[1]])
...
def build_padding_mask(token_ids, pad_id):
    ...
    mask = token_ids.unsqueeze(1).unsqueeze(1)
    return mask != pad_id
...
def build_causal_mask(seq_len):
    ...
    ones = torch.ones([1,1,seq_len,seq_len], dtype=torch.bool)
...
def combine_padding_and_causal_masks(padding_mask, causal_mask):
    return torch.mul(padding_mask, causal_mask)
...
def scale_attention_scores(scores, d_k):
    return torch.div(scores, math.sqrt(d_k))
...
def softmax_attention_weights(masked_scores):
    probs = torch.nn.functional.softmax(masked_scores, -1)
